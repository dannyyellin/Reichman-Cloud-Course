# This is example REST API server code for lecture: topic-3-docker
# This module, rest-word-svr-v2.py, augments the previous example, rest-word-svr-v1.py, to run in a Docker container. It
# differs from rest-word-svr-v1.py in that it removes app.run from __main__ so that the required parameters (such as
# the IP address and port) can be specified in Dockerfile.
...
# To run this program from a terminal window issue the cmds:
...
# If run from a Docker container, the cmds given above must be included in the Dockerfile.   An example Dockerfile is
# given in Dockerfile.setport
